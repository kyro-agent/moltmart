...
async def get_price(symbol: str, request: Request):
    ...
    if not payment_signature:
        ...
        return Response(
            ...
            status_code=402,
            headers={"PAYMENT-REQUIRED": create_payment_required_header(), "Content-Type": "application/json"},
        )
...
@app.get("/prices")
async def get_prices(request: Request, symbols: str = "btc,eth,sol"):
    """
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            results = {}
            for symbol, cg_id in zip(symbol_list, cg_ids, strict=False):
                if cg_id in data:
